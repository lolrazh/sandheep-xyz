...
export default function Error({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-16rem)] text-center px-4">
      <h1 className="text-6xl font-medium mb-4">Oops!</h1>
      ...
      <div className="flex gap-4 flex-wrap justify-center">
        <button 
          onClick={() => reset()}
          className="text-sm uppercase tracking-wider px-6 py-3 bg-foreground text-background hover:opacity-80 transition-opacity"
        >
          Try Again
        </button>
        <button 
          onClick={() => router.push('/')}
          className="text-sm uppercase tracking-wider px-6 py-3 border border-border/20 hover:border-foreground/60 transition-colors"
        >
          Go Home
        </button>
      </div>
